...
export default function AttendancePage(){
  ...
  const expiryBadge = (endDate, status) => {
    ...
    if (days <= 7) return <span className="badge badge-yellow flex items-center gap-1"><AlertTriangle className="w-3 h-3"/>{days}d left</span>;
  ...
  return(<div className="space-y-6 animate-in">
    ...
    <tbody className="divide-y divide-surface-100">{list.map(a=>(<tr key={a.id} className="hover:bg-surface-50 hover-row transition-colors">
      ...
      <td className="px-6 py-3 text-sm text-surface-600 hidden md:table-cell">{a.memberPhone||'—'}</td>
      ...
      <td className="px-6 py-3">{expiryBadge(a.subscriptionEndDate, a.subscriptionStatus)}</td>
      ...
      <td className="px-6 py-3">{!a.checkOutTime&&<button onClick={()=>doCheckOut(a.memberId)} className="btn-ghost text-red-600 text-xs"><LogOut className="w-3 h-3"/>Out</button>}</td>
    </tr>))}</tbody></table></div>
