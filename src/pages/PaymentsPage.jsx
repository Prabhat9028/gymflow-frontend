...
export default function PaymentsPage() {
  ...
  useEffect(() => {
    ...
    paymentApi.getAll(page, 20)
      .then(({ data }) => { setPayments(data.content); setTotalPages(data.totalPages); setTotal(data.totalElements); })
  ...
  return (
    <div className="space-y-6 animate-in">
      <div>
        ...
        <p className="text-surface-500 text-sm">{total} total transactions</p>
      </div>
      ...
      <div className="card overflow-hidden">
              ...
              {payments.map(p => (
                <tr key={p.id} className="hover:bg-surface-50">
                  ...
                  <td className="px-6 py-3">
                    <span className={`badge ${p.status === 'PAID' ? 'badge-green' : p.status === 'PENDING' ? 'badge-yellow' : 'badge-red'}`}>{p.status}</span>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        {!loading && payments.length === 0 && <p className="py-12 text-center text-surface-400">No payments yet</p>}
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-6 py-3 border-t bg-surface-50">
            ...
            <div className="flex gap-2">
              <button className="btn-ghost" disabled={page === 0} onClick={() => setPage(p => p - 1)}><ChevronLeft className="w-4 h-4" /></button>
              <button className="btn-ghost" disabled={page >= totalPages - 1} onClick={() => setPage(p => p + 1)}><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        )}
      </div>
