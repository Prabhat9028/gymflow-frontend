...
export default function MembersPage(){
  ...
  useEffect(()=>{load();},[load]);
  useEffect(()=>{biometricApi.getDevices().then(r=>setDevices(r.data)).catch(()=>{});planApi.getAll().then(r=>setPlans(r.data)).catch(()=>{});},[]);

  const emptyForm={firstName:'',lastName:'',email:'',phone:'',gender:'',dateOfBirth:'',address:'',emergencyContactName:'',emergencyContactPhone:'',
    planId:'',discountAmount:'',amountPaid:'',balanceAmount:'',balanceDueDate:'',paymentMode:'CASH'};
  const [form,setForm]=useState(emptyForm);
  const set=(k,v)=>setForm(p=>({...p,[k]:v}));

  // Auto-calculate balance when plan/discount/amountPaid change
  ...
  const handleSave=async e=>{
    ...
    const payload = {...form};
    if (form.planId) {
      payload.discountAmount = discount; payload.amountPaid = paid;
      payload.balanceAmount = balance;
    } else {
      delete payload.planId; delete payload.discountAmount; delete payload.amountPaid; delete payload.balanceAmount; delete payload.balanceDueDate; delete payload.paymentMode;
    }
    try{
      if(modal?.id){await memberApi.update(modal.id,payload);toast.success('Updated');}
      else{await memberApi.create(payload);toast.success('Member created'+(form.planId?' with plan':''));}
      setModal(null);load();
    }catch(err){toast.error(err.response?.data?.error||'Failed');}
  };
  const openNew=()=>{setForm(emptyForm);setModal({});};
  const openEdit=m=>{setForm({firstName:m.firstName,lastName:m.lastName,email:m.email||'',phone:m.phone||'',gender:m.gender||'',dateOfBirth:m.dateOfBirth||'',address:m.address||'',emergencyContactName:m.emergencyContactName||'',emergencyContactPhone:m.emergencyContactPhone||'',planId:'',discountAmount:'',amountPaid:'',balanceAmount:'',balanceDueDate:'',paymentMode:'CASH'});setModal(m);};
  ...
  return(<div className="space-y-6 animate-in">
    ...
    <div className="card overflow-hidden"><div className="overflow-x-auto"><table className="w-full"><thead><tr className="bg-surface-50 border-b">
      ...
      {members.map(m=>(<tr key={m.id} className="hover:bg-surface-50">
        ...
        <td className="px-6 py-3 text-sm hidden lg:table-cell">{m.activeSubscription?.plan?.name||<span className="text-surface-400">—</span>}</td>
        ...
        <td className="px-6 py-3"><div className="flex gap-1">
          {!m.biometricEnrolled&&<button onClick={e=>{e.stopPropagation();setEnrollModal(m);}} className="btn-ghost text-xs text-brand-600 hover:bg-brand-50"><Fingerprint className="w-3.5 h-3.5"/>Enroll</button>}
          <button onClick={e=>{e.stopPropagation();openEdit(m);}} className="btn-ghost text-xs">Edit</button>
        </div></td>
      </tr>))}
    </tbody></table></div>
    ...
    {/* Create/Edit Member Modal with Plan + Payment */}
    <Modal open={modal!==null} onClose={()=>setModal(null)} title={modal?.id?'Edit Member':'New Member'}>
      <form onSubmit={handleSave} className="p-6 space-y-4">
        ...
        {/* Plan & Payment Section (only for new members) */}
        {!modal?.id && (<>
          <div className="border-t pt-4 mt-2">
            <h4 className="font-display font-semibold text-sm text-surface-900 mb-3">Membership Plan & Payment</h4>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="sm:col-span-2">
                <label className="block text-sm font-medium text-surface-700 mb-1">Select Plan</label>
                <select className="input-field" value={form.planId} onChange={e=>set('planId',e.target.value)}>
                  <option value="">No plan (register only)</option>
              ...
              {form.planId && (<>
                ...
                <div className="sm:col-span-2 bg-surface-50 rounded-xl p-4 space-y-2">
                  ...
                  <div className="flex justify-between text-sm"><span className="text-surface-500">Discount</span><span className="font-semibold text-emerald-600">- ₹{discount.toLocaleString('en-IN')}</span></div>
                  <div className="border-t pt-2 flex justify-between text-sm"><span className="font-medium">Final Amount</span><span className="font-bold text-brand-600">₹{finalAmount.toLocaleString('en-IN')}</span></div>
                  <div className="flex justify-between text-sm"><span className="text-surface-500">Amount Paid</span><span className="font-semibold text-blue-600">₹{paid.toLocaleString('en-IN')}</span></div>
                  {balance > 0 && <div className="flex justify-between text-sm"><span className="text-red-600 font-medium">Balance Due</span><span className="font-bold text-red-600">₹{balance.toLocaleString('en-IN')}</span></div>}
                  {balance === 0 && paid > 0 && <div className="text-sm text-emerald-600 font-medium text-center pt-1">✓ Fully Paid</div>}
                </div>

                <div><label className="block text-sm font-medium text-surface-700 mb-1">Discount (₹)</label>
                  <input type="number" min="0" step="0.01" className="input-field" value={form.discountAmount} onChange={e=>set('discountAmount',e.target.value)} placeholder="0"/></div>
                <div><label className="block text-sm font-medium text-surface-700 mb-1">Amount Paid (₹) *</label>
                  <input type="number" min="0" step="0.01" className="input-field" value={form.amountPaid} onChange={e=>set('amountPaid',e.target.value)} placeholder={String(finalAmount)} required/></div>
                <div><label className="block text-sm font-medium text-surface-700 mb-1">Payment Mode</label>
                  <select className="input-field" value={form.paymentMode} onChange={e=>set('paymentMode',e.target.value)}>
                ...
                {balance > 0 && (
                  <div><label className="block text-sm font-medium text-surface-700 mb-1">Balance Due Date</label>
                    <input type="date" className="input-field" value={form.balanceDueDate} onChange={e=>set('balanceDueDate',e.target.value)}/></div>
                )}
              </>)}
            </div>
          </div>
        ...
        <div className="flex justify-end gap-3 pt-4">
          ...
          <button type="submit" className="btn-primary">{modal?.id?'Update':'Create Member'}</button>
        </div>
      </form>
    </Modal>
    ...
    <Modal open={!!enrollModal} onClose={()=>setEnrollModal(null)} title="Enroll Fingerprint" maxWidth="max-w-md">
      {enrollModal&&<div className="p-6 space-y-4">
        ...
        <div className="bg-surface-50 p-4 rounded-xl text-sm space-y-1"><p>1. Select device → 2. Click Enroll → 3. Scan finger on device</p></div>
