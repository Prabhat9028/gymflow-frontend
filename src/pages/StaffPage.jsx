...
import { Plus, Search, LogIn, LogOut, Clock, Trash2, UserCog, Camera } from 'lucide-react';
...
export default function StaffPage() {
  ...
  const [staffList, setStaffList] = useState([]); const [page, setPage] = useState(0); const [totalPages, setTotalPages] = useState(0); const [total, setTotal] = useState(0);
  const [search, setSearch] = useState(''); const [modal, setModal] = useState(null); const [loading, setLoading] = useState(true);
  const [todayAtt, setTodayAtt] = useState([]); const [checkInCode, setCheckInCode] = useState('');
  const [photoFile, setPhotoFile] = useState(null); const [uploading, setUploading] = useState(false);
  ...
  const [form, setForm] = useState({ firstName:'',lastName:'',email:'',password:'',phone:'',role:'STAFF',department:'',designation:'',salary:'',shiftStart:'',shiftEnd:'',photoUrl:'' });
  const set = (k,v) => setForm(p => ({ ...p, [k]: v }));
  const openNew = () => { setForm({ firstName:'',lastName:'',email:'',password:'',phone:'',role:'STAFF',department:'',designation:'',salary:'',shiftStart:'',shiftEnd:'',photoUrl:'' }); setPhotoFile(null); setModal({}); };

  const handlePhotoUpload = async (file) => {
    if (!file) return;
    setUploading(true);
    try { const { data } = await uploadApi.upload(file, 'staff'); set('photoUrl', data.url); toast.success('Photo uploaded'); }
  ...
  return (<div className="space-y-6 animate-in">
    ...
    {tab === 'profiles' && (<>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 stagger-in">
        {staffList.map(s => (<div key={s.id} className="card p-5 card-hover">
          <div className="flex items-start gap-4">
            {s.photoUrl ? <img src={s.photoUrl} alt="" className="w-12 h-12 rounded-xl object-cover flex-shrink-0"/>
    ...
    {tab === 'attendance' && (<>
      <div className="card p-6 animate-slide-up"><h3 className="font-display font-semibold mb-4">Staff Check-in</h3>
      ...
      <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b bg-surface-50/50">
        ...
        <td className="px-6 py-3 text-sm">{a.checkOutTime?format(new Date(a.checkOutTime),'hh:mm a'):<span className="text-brand-600 text-xs font-medium animate-pulse">On duty</span>}</td>
      ...
      <form onSubmit={handleSave} className="p-6 space-y-4">
        ...
        <div className="flex items-center gap-4">
          <div className="relative">
            {form.photoUrl ? <img src={form.photoUrl} alt="" className="w-20 h-20 rounded-xl object-cover"/>
            : <div className="w-20 h-20 rounded-xl bg-surface-100 flex items-center justify-center text-surface-400"><Camera className="w-8 h-8"/></div>}
            <label className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-brand-500 text-white flex items-center justify-center cursor-pointer hover:bg-brand-600 transition-colors shadow">
              <Camera className="w-3.5 h-3.5"/>
              <input type="file" accept="image/*" className="hidden" onChange={e=>{if(e.target.files[0])handlePhotoUpload(e.target.files[0])}} disabled={uploading}/>
            </label>
          </div>
          <div><p className="text-sm font-medium">Profile Photo</p><p className="text-xs text-surface-400">{uploading?'Uploading...':'Click camera to upload'}</p></div>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div><label className="block text-sm font-medium text-surface-700 mb-1">Email *</label><input type="email" className="input-field" value={form.email} onChange={e=>set('email',e.target.value)} required/></div>
          ...
          <div><label className="block text-sm font-medium text-surface-700 mb-1">Role</label><select className="input-field" value={form.role} onChange={e=>set('role',e.target.value)}><option value="STAFF">Staff</option><option value="ADMIN">Admin</option><option value="TRAINER">Trainer</option></select></div>
        ...
        <div className="flex justify-end gap-3 pt-4"><button type="button" onClick={()=>setModal(null)} className="btn-secondary">Cancel</button><button type="submit" className="btn-primary">Create</button></div>
      </form>
    </Modal>
  </div>);
}
