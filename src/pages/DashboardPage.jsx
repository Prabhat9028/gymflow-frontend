...
export default function DashboardPage(){
  ...
  return(<div className="space-y-6 animate-in">
    <div><h1 className="font-display text-2xl font-bold">Dashboard</h1><p className="text-surface-500 text-sm">Overview of your gym</p></div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="card p-6 animate-slide-up" style={{animationDelay:'0.1s'}}><h3 className="font-display font-semibold mb-4">Member Expiry Overview</h3>
        <p className="text-xs text-surface-400 mb-4">Expired (past) vs Expiring (future) members</p>
        <ResponsiveContainer width="100%" height={260}><BarChart data={s.expiryChart||[]}><XAxis dataKey="label" axisLine={false} tickLine={false} fontSize={11} tick={{fill:'#94a3b8'}}/><YAxis axisLine={false} tickLine={false} fontSize={12}/><Tooltip contentStyle={{borderRadius:12,border:'none',boxShadow:'0 4px 20px rgba(0,0,0,0.08)'}}/><Bar dataKey="value" name="Members" radius={[6,6,0,0]}>
          {(s.expiryChart||[]).map((_,i)=><Cell key={i} fill={EXPIRY_COLORS[i%EXPIRY_COLORS.length]}/>)}
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
      <div className="card animate-slide-up" style={{animationDelay:'0.3s'}}><div className="px-6 py-4 border-b"><h3 className="font-display font-semibold">Recent Members</h3></div><div className="divide-y">
        {(s.recentMembers||[]).slice(0,5).map(m=>(<div key={m.id} onClick={()=>nav(`/members/${m.id}`)} className="px-6 py-3 flex items-center justify-between hover:bg-surface-50 cursor-pointer transition-colors"><div className="flex items-center gap-3"><div className="w-8 h-8 rounded-full bg-brand-100 text-brand-700 flex items-center justify-center text-xs font-bold">{m.firstName?.[0]}{m.lastName?.[0]}</div><div><p className="text-sm font-medium">{m.firstName} {m.lastName}</p><p className="text-xs text-surface-400">{m.phone||m.memberCode}</p></div></div><span className={m.activeMembership?'badge badge-green':'badge badge-gray'}>{m.activeMembership?'Active':'—'}</span></div>))}
